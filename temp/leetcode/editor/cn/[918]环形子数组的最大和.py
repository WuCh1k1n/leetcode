...
解释：从子数组 [3] 和 [3,-2,2] 都可以得到最大和 3
 ...
 Related Topics 队列 数组 分治 动态规划 单调队列 👍 624 👎 0
...
class Solution:
    def maxSubarraySumCircular(self, nums: List[int]) -> int:
        ...
        for i in range(1, n):
            dp_max[i] = dp_max[i - 1] + nums[i] if dp_max[i - 1] > 0 else nums[i]
            dp_min[i] = dp_min[i - 1] + nums[i] if dp_min[i - 1] < 0 else nums[i]
        ret = max(dp_max)
        if max(nums) > 0:
            ret = max(ret, sum(nums) - min(dp_min))
        return ret
